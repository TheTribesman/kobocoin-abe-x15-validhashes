Kobocoin-Abe setup for MySQL: 8 easy steps.
...
6. Create file abe-load.conf with the following contents, replacing
"PASSWORD" as above:
...
7. Perform the initial data load:
    ...
    python -m Kobocoin-Abe.abe --config abe-load.conf --commit-bytes 100000 --no-serve
...
8. Then run the web server as:
    ...
    python -m Kobocoin-Abe.abe --config abe-load.conf
...
database. Also please note that you should not compress tables while Kobocoin-Abe is
running.
...
and default settings for firtbits and scripsig. Compression of a full Kobocoin-Abe
